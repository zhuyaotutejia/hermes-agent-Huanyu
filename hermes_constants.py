...
def resolve_reasoning_config(cfg: "dict | None", model: str = "") -> "dict | None":
    ...
    Single chokepoint for reasoning-effort resolution. Priority:
    ...
    Args:
        cfg: A loaded config dict (only the ``agent`` and ``model``
             sections are read).
        model: The effective model for this surface/session. When empty,
               it is derived from the config's ``model`` section.
